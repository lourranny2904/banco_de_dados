...
def create_tarefa():
    if request.method == 'POST':
        ...
        cursor = conexao.connection.cursor()
        cursor.execute("INSERT INTO tarefas (usuario_id, titulo, conteudo, data, limit_data, status) VALUES (%s, %s, %s, %s, %s, %s)", (current_user.id, titulo, conteudo, data, limit_data, 'Em andamento'))        
        conexao.connection.commit()
        cursor.close()
