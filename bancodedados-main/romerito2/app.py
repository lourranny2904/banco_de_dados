...
def create_tarefa():
    if request.method == 'POST':
        titulo = request.form['nome']  # Corrigido de 'nome' para 'titulo'
        ...
        cursor = conexao.connection.cursor()
        cursor.execute("""
            INSERT INTO tarefas (usuario_id, titulo, conteudo, data, limit_data, status, prioridade, categoria) 
            VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
        """, (current_user.id, titulo, conteudo, data, limit_data, status, prioridade, categoria))
...
def ver_tarefa():
    if request.method == 'POST':
        ...
        cursor = conexao.connection.cursor()
        query = ("INSERT INTO tarefas (usuario_id, titulo, conteudo, data, limit_data, status, prioridade, categoria) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)", 
                 (current_user.id, titulo, conteudo, data, limit_data, status, prioridade, categoria))
...
def editar(id):
    ...
    if request.method == 'POST':
        if 'titulo' in request.form and 'conteudo' in request.form:
            ...
            cursor.execute('UPDATE tarefas SET titulo = %s, conteudo = %s, limit_data = %s, status = %s, prioridade = %s, categoria = %s WHERE id = %s', 
               (titulo, conteudo, limit_data, status, prioridade, categoria, id))
            conexao.connection.commit()
            cursor.close()
